<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <meta name="keywords" content="jquery,ui,easy,easyui,web">
        <meta name="description" content="">
        <title>GMI Gloria</title>
        <link rel="stylesheet" type="text/css" href="<?= base_url();?>libraries/css/style.css">
        <link rel="stylesheet" type="text/css" href="<?= base_url();?>libraries/jquery-easyui/155/themes/default/easyui.css">

        <link rel="stylesheet" type="text/css" href="<?= base_url();?>libraries/jquery-easyui/155/themes/icon.css">
        <link rel="stylesheet" type="text/css" href="<?= base_url();?>libraries/jquery-easyui/155/themes/color.css">

        <script type="text/javascript" src="<?= base_url();?>libraries/jquery-easyui/155/jquery.min.js"></script>
        <script type="text/javascript" src="<?= base_url() ?>libraries/js/autoNumeric.js"></script>
        <script type="text/javascript" src="<?= base_url();?>libraries/jquery-easyui/155/jquery.easyui.min.js"></script>
        <script type="text/javascript" src="<?= base_url() ?>libraries/jquery-easyui/155/datagrid-filter.js"></script>
        <script type="text/javascript" src="<?= base_url() ?>libraries/jquery-easyui/155/columns-ext.js"></script>
        <script type="text/javascript" src="<?= base_url() ?>libraries/js/main.js"></script>

    </head>