<script type="text/javascript">
    ...
    function newData(){
        $('#dlg').dialog({
            closed:false,
            title:'Tambah data',
            href:'<?php echo base_url(); ?>serving/add',
            onLoad:function(){
                 url = '<?= base_url() ?>serving/add';
                 oper="";
    ...
    function editData(parameter_key){
        ...
        if (row!=''){
            $('#dlg').dialog({
                closed:false,
                title:'Edit serving',
                href:'<?php echo base_url(); ?>serving/edit/'+row,
                onLoad:function(){
                    url = '<?= base_url() ?>serving/edit/'+row;
                    oper="";
                    $("#btnSave span span.l-btn-text").text("Save");
                }
            });
    ...
    function viewData(parameter_key){
        ...
        if (row!=''){
            $('#dlgView').dialog({
                closed:false,
                title:'View data',
                href:'<?php echo base_url(); ?>serving/view/'+row
            });
    ...
    function deleteData(parameter_key){
        ...
        if (row!=''){
            $('#dlg').dialog({
                closed:false,
                title:'Delete data',
                href:'<?php echo base_url(); ?>serving/delete/'+row,
                onLoad:function(){
                    url = '<?= base_url() ?>serving/delete/'+row;
                    oper="del";
...
<div class="easyui-tabs" style="height:auto">
    <div title="Data serving" style="padding:10px">
         <table id="dg" title="serving" class="easyui-datagrid" style="width:100%;height:250px" url="<?= $link ?>"
                >
            <thead>
                <tr>
                    <th field="aksi" width="5%">Aksi</th>
                    <th field="parameter_key" width="10%" hidden="true"></th>
                    <th field="parameterid" width="5%" sortable="true">servingid</th>
                    <th field="parametertext" width="5%" sortable="true">serving</th>
                    <th field="parametermemo" width="5%" sortable="true">servingmemo</th>
