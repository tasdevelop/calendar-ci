<script type="text/javascript">
    ...
    function newData(){
        $('#dlg').dialog({
            closed:false,
            title:'Tambah data',
            href:'<?php echo base_url(); ?>persekutuan/add',
            onLoad:function(){
                 url = '<?= base_url() ?>persekutuan/add';
                 oper="";
    ...
    function editData(parameter_key){
        ...
        if (row!=''){
            $('#dlg').dialog({
                closed:false,
                title:'Edit persekutuan',
                href:'<?php echo base_url(); ?>persekutuan/edit/'+row,
                onLoad:function(){
                    url = '<?= base_url() ?>persekutuan/edit/'+row;
                    oper="";
                    $("#btnSave span span.l-btn-text").text("Save");
                }
            });
    ...
    function viewData(parameter_key){
        ...
        if (row!=''){
            $('#dlgView').dialog({
                closed:false,
                title:'View data',
                href:'<?php echo base_url(); ?>persekutuan/view/'+row
            });
    ...
    function deleteData(parameter_key){
        ...
        if (row!=''){
            $('#dlg').dialog({
                closed:false,
                title:'Delete data',
                href:'<?php echo base_url(); ?>persekutuan/delete/'+row,
                onLoad:function(){
                    url = '<?= base_url() ?>persekutuan/delete/'+row;
                    oper="del";
...
<div class="easyui-tabs" style="height:auto">
    <div title="Data persekutuan" style="padding:10px">
         <table id="dg" title="persekutuan" class="easyui-datagrid" style="width:100%;height:250px" url="<?= $link ?>"
                >
            <thead>
                <tr>
                    <th field="aksi" width="5%">Aksi</th>
                    <th field="parameter_key" width="10%" hidden="true"></th>
                    <th field="parameterid" width="5%" sortable="true">persekutuanid</th>
                    <th field="parametertext" width="5%" sortable="true">persekutuan</th>
                    <th field="parametermemo" width="5%" sortable="true">persekutuanmemo</th>
